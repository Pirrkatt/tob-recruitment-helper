...
- Rename everything from the template. Do not leave `com.example`, `TobRecruitHelperPlugin`, `TobRecruitHelperConfig`, or `example` as the config group. Rename the package path, class names, config group, `build.gradle` group, `settings.gradle` project name, and `runelite-plugin.properties`.
